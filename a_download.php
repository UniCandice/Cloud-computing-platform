...
$cmd="cd ".PYTHON_PATH.";python solver.py -d ".$z;
exec($cmd);
...
$tcase_table='tcase'.$z;
echo '<table class="downpath"><tr><td>path</td><td>'.$downpath.'</td></tr><tr><td>testcase</td><td><dl>';
$result1=mysql_query("select * from ".$tcase_table);
while($row1=mysql_fetch_array($result1)){
    echo '<dt class="dl_download">'.'<a href="'.$downpath.$row1[Case_name].'.tar.gz" style="text-decoration:none">'.$row1[Case_name].'</a></dt>';
}
echo '</dl></td></tr></table>';
mysql_close($con);
?>
