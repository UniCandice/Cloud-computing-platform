...
@author: ssheng
"""
from sts_class import Config19
...
testcase=os.path.join(testpath,r'testcase19.csv')
...
def tran():
    ...
    sourceFile=os.path.join(constant.path['resource'],'conv_cgns')
    if not os.path.exists(testpath):
        os.makedirs(testpath)
    shutil.copy(sourceFile,testpath)
    
    cmd="cd "+testpath+r";chmod 777 conv_cgns;./conv_cgns "+Cgns_case+r".cgns"
    os.system(cmd)
    
    sym_dat=os.path.join(testpath,'symmetry.dat')
    inp_dat=os.path.join(testpath,'inplot.dat')
    wall_dat=os.path.join(testpath,'wall.dat')
    
    f3=open(inp_dat,'w')
    
    with open(sym_dat) as f1:
        l1=f1.readlines()[0]
    with open(wall_dat) as f2:
        l2=f2.readlines()[0]
    f1.close()
    f2.close()
...
def cp():
#   将ch19求解器及bhcfd.grd/bhcfd.bc/inplot.dat文件拷贝到目标区域
    global testpath
    global testcase
    ...
    file1+=glob.glob(testpath+os.sep+r'bhcfd.bc')
    file1+=glob.glob(testpath+os.sep+r'inplot.dat')
    file1+=glob.glob(testpath+os.sep+r'bhcfd.grd')
        ...
        if row['dotask(Yes=1)']=='1':
            ...
            for files in os.listdir(sourcedir):
                ...
                if os.path.isfile(sourceFile):
                    ...
                    if sourceFile==file1[0] or sourceFile==file1[1] or sourceFile==file1[2] or sourceFile==file1[3]:
...
#        runflag=True     
    ...
    .. help::
    配置文件bhcfd19.sts.template
    python solver19.py  -r(或者：--run，第一次提状态使用,相应的生成run.sh/bhcfd.sts,然后cp,run_project)
    python solver19.py  -g(任何时候可单独使用，相应的生成run.sh/bhcfd.sts)
    python solver19.py  -x(或者：--rerun，sts更改istart,cfl,nstep,并提交)
    python solver19.py  -s --list=*(--san，删除计算文件，list指定后缀，*全删除)
    python solver19.py  -k(或者：--kill，停状态)
    python solver19.py  -c(或者：--check)
    python solver19.py  -u --list=bhcfd.sts(--upload,将filetocopy中list指定名字上传)
                    多个文件：    --list=bhcfd.sts,bhcfd.grd,bhcfd.bc
    python solver19.py  -d --list=FCONV,ECONV  (--download,list指定后缀下载文件)
    python solver19.py  -p (生成力曲线图、残差收敛图)
