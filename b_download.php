...
mysql_close($con);
if($y=='ch20'){
  $cmd="cd ".PYTHON_PATH.";python solver20.py -d ".$z;
  exec($cmd);
}
elseif($y=='ch19'){
  $cmd="cd ".PYTHON_PATH.";python solver19.py -d ".$z;
  exec($cmd);
}
...
for($i=1;$i<=$number;$i++){
    echo '<dt class="dl_download">'.'<a href="'.$downpath.$casename.$i.'.tar.gz" style="text-decoration:none">'.$casename.$i.'</a></dt>';
}
echo '</dl></td></tr></table>';
?>
</body>
